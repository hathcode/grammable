...
RSpec.describe GramsController, type: :controller do
  describe "grams#destroy" do
    it "shouldn't allow a user who didn't create a gram destroy it" do
  ...
  describe "grams#update" do
    it "shouldn't let a user who did not create the gram update the gram" do
      ...
      user = FactoryGirl.create(:user)
      sign_in user
      patch :update, id: p.id, gram: {message: 'wahoooo'}
      expect(response).to have_http_status(:forbidden)
    end

    it "shouldn't allow unauthenticated users to create a gram" do
      p = FactoryGirl.create(:gram)
